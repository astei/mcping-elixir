defmodule MCPing.SrvTest do
  ...
  test "single SRV records work" do
    assert MCPing.Srv.find_first_eligible_srv_record([{1, 1, 25565, "example.com"}]) ==
             {"example.com", 25565}
  end

  test "will pick the SRV record with the lowest priority first" do
    assert MCPing.Srv.find_first_eligible_srv_record([
             {1, 1, 25566, "s2.example.com"},
             {0, 1, 25565, "s1.example.com"}
           ]) == {"s1.example.com", 25565}
  end
  ...
  test "will respect SRV record weight" do
    ...
    results =
      Enum.map(1..100, fn _ ->
        {v, state} =
          MCPing.Srv.find_first_eligible_srv_record(records, Process.get(:srv_test_state))
